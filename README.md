...
v0.3.25 修复普通用户升级时旧 CADFlow 进程未响应 TERM 导致更新中止的问题：先优雅停止，超时后仅终止已记录且已校验的 CADFlow PID。v0.3.24 在队列名称上增加悬停提示，展示 LSF 配置的节点成员；`HOSTS: all` 会展开为当前采集到的全部节点，无法取得队列长描述时明确显示未提供。v0.3.23 在 PEND/PSUSP 作业状态下显示 LSF 返回的等待/挂起原因，并在总览等待作业预览中同步展示原因；无法取得原因时保持空值，不影响其他采集数据。
