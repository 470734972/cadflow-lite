...
v0.3.27 队列节点提示会调用只读 `bmgroup -r -w`，将 LSF 主机组递归展开为实际服务器；无法展开时保留原组名，避免误报。v0.3.26 加强升级停止保护：TERM 超时后再次校验记录 PID 仍是 CADFlow 进程，确认未发生 PID 复用才允许 KILL。v0.3.25 修复普通用户升级时旧 CADFlow 进程未响应 TERM 导致更新中止的问题：先优雅停止，超时后仅终止已记录且已校验的 CADFlow PID。v0.3.24 在队列名称上增加悬停提示，展示 LSF 配置的节点成员；`HOSTS: all` 会展开为当前采集到的全部节点，无法取得队列长描述时明确显示未提供。v0.3.23 在 PEND/PSUSP 作业状态下显示 LSF 返回的等待/挂起原因，并在总览等待作业预览中同步展示原因；无法取得原因时保持空值，不影响其他采集数据。

v0.3.22 将总览等待作业预览改为 KPI 区域内的展开面板，避免悬停详情遮挡资源利用率图表。

v0.3.28 在页面右上角显示最近一次采集时间，并可悬停查看原始时间戳。
